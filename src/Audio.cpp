...
void Audio::processLocalFile() {
    ...
    if (m_resumeFilePos >= 0) { // we have a resume file position
        ...
        m_audioDataReadPtr = (m_prlf.newFilePos > (int32_t)m_audioDataStart) ? (m_prlf.newFilePos - m_audioDataStart) : 0;
        m_resumeFilePos = -1;
        m_f_allDataReceived = false;
        return;
    }
...
void Audio::processWebFile() {
    ...
    if (m_resumeFilePos >= 0) { // we have a resume file position
        ...
        m_audioDataReadPtr = (m_pwf.newFilePos > (int32_t)m_audioDataStart) ? (m_pwf.newFilePos - m_audioDataStart) : 0;
        m_resumeFilePos = -1;
        m_f_allDataReceived = false;
        return;
    }
...
void Audio::playAudioData() {
    ...
    xSemaphoreTake(mutex_audioTaskIsDecoding, 1 * configTICK_RATE_HZ);
    {
        ...
        if (isFile) {
            ...
            size_t audioBytesRemaining = (m_audioDataReadPtr < m_audioDataSize) ? (m_audioDataSize - m_audioDataReadPtr) : 0;
